...
export class HNSWVectorIndex {
    constructor(dimensions, storagePath, config) {
        this.index = null;
    ...
    loadFromDisk() {
        ...
        try {
            ...
            this.entries = new Map(data.entries.map((e) => [e[0], e[1]]));
            this.idToInternal = new Map(data.idToInternal);
            this.internalToId = new Map(data.internalToId.map(([k, v]) => [k, v]));
    ...
    addVector(chunkId, embedding, entry) {
        if (!this.index) {
            this.initialize();
        }
    ...
    updateVector(chunkId, embedding, entry) {
        if (!this.index) {
            this.initialize();
        }
    ...
    searchWithPathFilter(queryEmbedding, topK, pathPrefix) {
...
export function getOrCreateIndex(storagePath, dimensions, config) {
...
export function closeIndex(storagePath) {
