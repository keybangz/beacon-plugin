 ...
 */
export async function benchmarkSearch(db, embedder, config, queries, iterations = 3) {
    const results = [];
    for (const query of queries) {
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            // Execute search
            const searchResults = db.search(embedding, 10, 0.5, query, config);
 ...
 */
export async function benchmarkDatabase(db, config) {
    const results = [];
 ...
 */
export function generatePerformanceReport(results, title = "Beacon Performance Report") {
    let report = `\n${"=".repeat(50)}\n`;
    report += `${title}\n`;
