    ...
    async indexFiles(filePaths) {
        ...
        if (!this.useEmbeddings) {
            for (const file of filesToProcess) {
                ...
                if (chunks.length > 0) {
                    try {
                        const placeholderEmbeddings = chunks.map(() => this.generatePlaceholderEmbedding(file.content));
                        this.db.insertChunks(file.path, chunks.map((c) => ({
                            text: c.text,
                            start_line: c.start_line,
                            end_line: c.end_line,
                        })), placeholderEmbeddings, file.hash);
                        filesProcessed++;
        ...
        const embeddings = new Array(totalChunks).fill(null);
